...
WalletModel::WalletModel(CWallet *wallet, OptionsModel *optionsModel, QObject *parent) :
    ...
    cachedBalance(0), cachedBeepMagnetBal(0), cachedStake(0), cachedUnconfirmedBalance(0), cachedImmatureBalance(0),
...
qint64 WalletModel::getBeepMagnetBalance() const
{
    return wallet->GetBeepMagnetBalance();
}
...
void WalletModel::checkBalanceChanged()
{
    ...
    qint64 newBeepMagnetBal = getBeepMagnetBalance();
    qint64 newStake = getStake();
    qint64 newUnconfirmedBalance = getUnconfirmedBalance();
    qint64 newImmatureBalance = getImmatureBalance();

    if (cachedBalance != newBalance
        || cachedBeepMagnetBal != newBeepMagnetBal
        ...
        || cachedImmatureBalance != newImmatureBalance)
    {
        ...
        cachedUnconfirmedBalance = newUnconfirmedBalance;
        cachedImmatureBalance = newImmatureBalance;
        emit balanceChanged(newBalance, newBeepMagnetBal, newStake, newUnconfirmedBalance, newImmatureBalance);
    }
}
...
WalletModel::SendCoinsReturn WalletModel::sendCoinsAnon(const QList<SendCoinsRecipient> &recipients, const CCoinControl *coinControl)
{
    ...
    if (vNodes.empty())
        return SendCoinsReturn(SCR_ErrorWithMsg, 0, QString::fromStdString("BeepMagnet is not connected!"));
    ...
    {
        nBalance = wallet->GetBeepMagnetBalance();
        if ((nTotalOut + nTransactionFee) > nBalance)
            return SendCoinsReturn(SCR_AmountWithFeeExceedsBeepMagnetBalance, nTransactionFee);
    };
