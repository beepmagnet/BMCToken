...
#include "beepmagnetbridge.moc"

BeepMagnetBridge::BeepMagnetBridge(BeepMagnetGUI *window, QObject *parent) :
    QObject         (parent),
    window          (window),
...
BeepMagnetBridge::~BeepMagnetBridge()
{
...
void BeepMagnetBridge::setClientModel()
{
...
// This is just a hook, we won't really be setting the model...
void BeepMagnetBridge::setWalletModel()
{
...
// This is just a hook, we won't really be setting the model...
void BeepMagnetBridge::setMessageModel()
{
    populateMessageTable();
    connectSignals();
}

void BeepMagnetBridge::copy(QString text)
{
    QApplication::clipboard()->setText(text);
}

void BeepMagnetBridge::paste() {
    emitPaste(QApplication::clipboard()->text());
}

// Options
void BeepMagnetBridge::populateOptions()
{
...
// Transactions
bool BeepMagnetBridge::addRecipient(QString address, QString label, QString narration, qint64 amount, int txnType, int nRingSize)
{
...
void BeepMagnetBridge::clearRecipients()
{
    recipients.clear();
}

bool BeepMagnetBridge::sendCoins(bool fUseCoinControl, QString sChangeAddr)
{
    ...
    {
        int inputType; // 0 OK, 1 BeepMagnet
        switch(rcp.txnTypeInd)
        {
            case TXT_OK_TO_OK:
                formatted.append(tr("<b>%1</b> to %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::BMCoin, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                break;
            case TXT_OK_TO_ANON:
                formatted.append(tr("<b>%1</b> to BEEPMAGNET %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::BMCoin, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_ANON:
                formatted.append(tr("<b>%1</b>, ring size %2 to BEEPMAGNET %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::BMCoin, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_OK:
                formatted.append(tr("<b>%1</b>, ring size %2 to OK %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::BMCoin, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                break;
            default:
    ...
    switch(sendstatus.status)
    {
        ...
        case WalletModel::AmountWithFeeExceedsBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                arg(BitcoinUnits::formatWithUnit(BitcoinUnits::BMCoin, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        ...
        case WalletModel::SCR_StealthAddressFail:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        case WalletModel::SCR_AmountWithFeeExceedsBeepMagnetBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                tr("The total exceeds your beepmagnet balance when the %1 transaction fee is included.").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::BMCoin, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
...
void BeepMagnetBridge::openAddressBook(bool sending)
{
...
void BeepMagnetBridge::openCoinControl()
{
...
void BeepMagnetBridge::updateCoinControlAmount(qint64 amount)
{
    CoinControlDialog::payAmounts.clear();
...
void BeepMagnetBridge::updateCoinControlLabels(unsigned int &quantity, int64_t &amount, int64_t &fee, int64_t &afterfee, unsigned int &bytes, QString &priority, QString low, int64_t &change)
{
    emitCoinControlUpdate(quantity, amount, fee, afterfee, bytes, priority, low, change);
}

QVariantMap BeepMagnetBridge::listAnonOutputs()
{
...
void BeepMagnetBridge::populateTransactionTable()
{
...
void BeepMagnetBridge::updateTransactions(QModelIndex topLeft, QModelIndex bottomRight)
{
...
void BeepMagnetBridge::insertTransactions(const QModelIndex & parent, int start, int end)
{
    // New Transactions...
    transactionModel->populateRows(start, end);
}

QString BeepMagnetBridge::transactionDetails(QString txid)
{
...
// Addresses
void BeepMagnetBridge::populateAddressTable()
{
...
void BeepMagnetBridge::updateAddresses(QModelIndex topLeft, QModelIndex bottomRight)
{
    addressModel->poplateRows(topLeft.row(), bottomRight.row());
}

void BeepMagnetBridge::insertAddresses(const QModelIndex & parent, int start, int end)
{
...
QString BeepMagnetBridge::newAddress(bool own)
{
...
QString BeepMagnetBridge::getAddressLabel(QString address)
{
    return addressModel->atm->labelForAddress(address);
}

void BeepMagnetBridge::updateAddressLabel(QString address, QString label)
{
...
bool BeepMagnetBridge::validateAddress(QString address)
{
    return window->walletModel->validateAddress(address);
}

bool BeepMagnetBridge::deleteAddress(QString address)
{
    return addressModel->atm->removeRow(addressModel->atm->lookupAddress(address));
}

// Messages
void BeepMagnetBridge::appendMessages(QString messages, bool reset)
{
    emitMessages("[" + messages + "]", reset);
}

void BeepMagnetBridge::appendMessage(int row) {
...
void BeepMagnetBridge::populateMessageTable()
{
...
void BeepMagnetBridge::insertMessages(const QModelIndex & parent, int start, int end)
{
    while(start <= end)
    {
        appendMessage(start++);
...
bool BeepMagnetBridge::deleteMessage(QString key)
{
    return window->messageModel->removeRow(thMessage->mtm->lookupMessage(key));
}

bool BeepMagnetBridge::markMessageAsRead(QString key)
{
    return window->messageModel->markMessageAsRead(key);
}

QString BeepMagnetBridge::getPubKey(QString address, QString label)
{
    if(!label.isEmpty())
...
bool BeepMagnetBridge::setPubKey(QString address, QString pubkey)
{
...
bool BeepMagnetBridge::sendMessage(const QString &address, const QString &message, const QString &from)
{
    WalletModel::UnlockContext ctx(window->walletModel->requestUnlock());
...
void BeepMagnetBridge::connectSignals()
{
...
QVariantMap BeepMagnetBridge::userAction(QVariantMap action)
{
...
// Blocks
QVariantMap BeepMagnetBridge::listLatestBlocks()
{
...
QVariantMap BeepMagnetBridge::findBlock(QString searchID)
{
...
QVariantMap BeepMagnetBridge::blockDetails(QString blkHash)
{
...
QVariantMap BeepMagnetBridge::listTransactionsForBlock(QString blkHash)
{
...
QVariantMap BeepMagnetBridge::txnDetails(QString blkHash, QString txnHash)
{
