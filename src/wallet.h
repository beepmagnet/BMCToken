...
public:
    ...
    int64_t GetBeepMagnetBalance() const;
    ...
    int64_t GetBeepMagnetDebit(const CTxIn& txin) const;
    int64_t GetBeepMagnetCredit(const CTxOut& txout) const;
    ...
    int64_t GetDebit(const CTransaction& tx) const
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, tx.vin)
        {
            if (tx.nVersion == ANON_TXN_VERSION
                && txin.IsAnonInput())
            {
                nDebit += GetBeepMagnetDebit(txin);
            } else
    ...
    int64_t GetCredit(const CTransaction& tx) const
    {
        ...
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
        {
            if (tx.nVersion == ANON_TXN_VERSION
                && txout.IsAnonOutput())
            {
                nCredit += GetBeepMagnetCredit(txout);
            } else
    ...
    bool GetCredit(const CTransaction& tx, int64_t& nOK, int64_t& nBeepMagnet) const
    {
        nOK = 0;
        nBeepMagnet = 0;
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
        {
            if (tx.nVersion == ANON_TXN_VERSION
                && txout.IsAnonOutput())
            {
                nBeepMagnet += GetBeepMagnetCredit(txout);
            } else
                nOK += GetCredit(txout);
            if (!MoneyRange(nOK)
                || !MoneyRange(nBeepMagnet))
                throw std::runtime_error("CWallet::GetCredit() : value out of range");
        }
        return true;
    };
...
public:
    ...
    mutable bool fAvailableBeepMagnetCreditCached;
    mutable bool fCreditSplitCached;
    mutable int64_t nDebitCached;
    mutable int64_t nCreditCached;
    mutable int64_t nAvailableCreditCached;
    mutable int64_t nChangeCached;
    mutable int64_t nAvailableBeepMagnetCreditCached;
    
    mutable int64_t nCredOKCached;
    mutable int64_t nCredBeepMagnetCached;
    ...
    void Init(const CWallet* pwalletIn)
    {
        ...
        fAvailableBeepMagnetCreditCached = false;
        
        nCredOKCached = 0;
        nCredBeepMagnetCached = 0;
        ...
        nAvailableBeepMagnetCreditCached = 0;
        nChangeCached = 0;
        nOrderPos = -1;
    }
    ...
    bool GetCredit(int64_t& nCredOK, int64_t& nCredBeepMagnet, bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
        {
            nCredOK = nCredBeepMagnet = 0;
            return true;
        }
        ...
        if (!fUseCache || !fCreditSplitCached)
        {
            nCredOKCached = 0;
            nCredBeepMagnetCached = 0;
            pwallet->GetCredit(*this, nCredOKCached, nCredBeepMagnetCached);
            fCreditSplitCached = true;
        };
        
        nCredOK = nCredOKCached;
        nCredBeepMagnet = nCredBeepMagnetCached;
        return true;
    }
    ...
    int64_t GetAvailableBeepMagnetCredit(bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
            return 0;
        
        if (fUseCache && fAvailableBeepMagnetCreditCached)
            return nAvailableBeepMagnetCreditCached;
        ...
        for (unsigned int i = 0; i < vout.size(); i++)
        {
            if (!IsSpent(i))
            {
                ...
                if (!MoneyRange(nCredit))
                    throw std::runtime_error("CWalletTx::GetAvailableBeepMagnetCredit() : value out of range");
            };
        };
        
        nAvailableBeepMagnetCreditCached = nCredit;
        fAvailableBeepMagnetCreditCached = true;
        return nCredit;
    };
