...
void ThreadIRCSeed()
{
    ...
    for(;;)
    {
        ...
        if (fTestNet)
        {
            Send(hSocket, "JOIN #beepmagnetTEST\r");
            Send(hSocket, "WHO #beepmagnetTEST\r");
        } else
        {
            // randomly join #beepmagnet00-#beepmagnet05
            //int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #beepmagnet%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #beepmagnet%02d\r", channel_number).c_str());
        };
